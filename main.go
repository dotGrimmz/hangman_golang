...
type GameData struct {
	selectedTheme, wordToGuess, displayValue, alreadyGuessed, userGuess string
...
func main() {
	...
	for {
		...
		userQuit := gameData.userGuess == "QUIT"
...
func handleUserGuess(game *GameData) {
	...
	game.userGuess = current
	if game.userGuess == "QUIT" {
		fmt.Println("Sorry to see you go!")
		return
	}

	if len(game.userGuess) > 1 {
...
func getStartInto(game *GameData) {
	...
	file, err := ioutil.ReadFile("data/difficultyLevels.json")
	if err != nil {
		panic(err)
	}

	err = json.Unmarshal(file, &difficultyLevels)
	if err != nil {
		panic(err)
	}
